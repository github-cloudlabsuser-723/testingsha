...
1. The `resources` table is queried to filter rows based on the resource type. The query selects rows where the type matches 'Microsoft.Sql/servers', 'microsoft.azurearcdata/sqlserverinstances', or 'microsoft.sql/managedinstances'.

2. The `id` field is extended to store the identifier of each resource.

3. The `sqlprop` field is extended to convert the `properties` field to a string.

4. Additional fields such as `propertiesState`, `propertiesStatus`, `version`, `patch`, `vcore`, `license`, `vcores`, and `edition` are extended to extract specific information from the `properties` field.

5. The `state` field is extended using a `case` statement to assign values based on the `propertiesState` and `propertiesStatus` fields. The possible values for `state` include 'Degraded', 'Warned', 'Disabled', 'Available', 'Reserved', 'Connected', or an empty string if `propertiesState` is empty.

6. The `Cores` field is extended using another `case` statement to determine the number of cores based on the values of `vcore`, `vcores`, and `propertiesState`.

7. The `SQLversion` field is extended using a nested `case` statement to determine the SQL Server version based on the `version` and `properties.patchLevel` fields. The version is identified based on specific values of the `patchLevel` field, and if no match is found, the value is set to "Unknown".

The code essentially retrieves information about SQL Servers, including their IDs, properties, states, versions, patches, vCores, licenses, and editions. It applies conditional logic to determine and assign values based on the extracted information.
...
// This file is a kusto query that will lsit all SQL Servers registered with ARC Extended SQL including those in GCP and AWS.
	...
	| extend SQLversion = case( substring(version, 0, 2) == "16", 
									case(substring(properties.patchLevel,5,4) == "4025", "SQL 2022 (CU3)",
...
| extend mnf = tostring(properties.detectedProperties.manufacturer)
| extend mod = tostring(properties.detectedProperties.model)
| extend vmprop = tostring(properties)) on name
	...
	| project id, name, location, mnf, mod, SQLversion,  edition, license, Cores
